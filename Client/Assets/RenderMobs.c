...
struct rr_renderer_spritesheet friendly_mob_sprites[21];
    ...
    if (flags & 1)
    {
        if (flags & 2)
        {
            if (id == rr_mob_id_trex)
                ...
                render_sprite_from_cache(renderer, &friendly_mob_sprites[20],
                                         pos);
    ...
    case rr_mob_id_house_centipede:
        ...
        break;
          case rr_mob_id_lanternfly:
            ...
            rr_renderer_context_state_init(renderer, &state);
            rr_renderer_rotate(renderer, animation_tick * 0.2f - M_PI / 6);
            rr_renderer_translate(renderer, 0, 75);
            rr_renderer_scale2(renderer, -1, 1);
            ...
            rr_renderer_context_state_init(renderer, &state);
            rr_renderer_rotate(renderer, animation_tick * 0.2f - M_PI / 6 + M_PI);
            rr_renderer_translate(renderer, 0, 75);
            rr_renderer_rotate(renderer, M_PI / 20);
            render_sprite(renderer, id, 2, flags);
            rr_renderer_context_state_free(renderer, &state);
            rr_renderer_context_state_init(renderer, &state);
            rr_renderer_rotate(renderer, animation_tick * -0.2f + M_PI);
            rr_renderer_translate(renderer, 0, 75);
            rr_renderer_rotate(renderer, M_PI / 20);
            render_sprite(renderer, id, 2, flags);
            rr_renderer_context_state_free(renderer, &state);
...
void rr_renderer_mob_cache_init()
{
    ...
    rr_renderer_spritesheet_init(
        ...
        rr_lanternfly_wing1_draw, 0);
            ...
            rr_renderer_spritesheet_init(
    &friendly_mob_sprites[20], friendly_mask, 240, 144, rr_pectinodon_head_draw, 336, 192,
